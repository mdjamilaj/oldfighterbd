...
<div>
	<div v-if="notice" class="bg-green-100">
		<center>
			<div>
				<marquee behavior="scroll" direction="left" onmouseover="this.stop();" onmouseout="this.start();" id="MARQUEE1" class="scrolling"><b class="text-info">NOTICE : <span class="text-red-600"> {{ notice.notice }}</span></b></marquee>
			</div>
		</center>
	</div>
		  ...
		  <div x-data="{ open: false }" class="flex flex-col max-w-screen-xl px-4 mx-auto md:items-center md:justify-between md:flex-row md:px-6 lg:px-8">
		    <div class="flex flex-row items-center justify-between">
		      	...
		      	<router-link :to="'/'" class="text-lg font-semibold tracking-widest text-gray-900 uppercase rounded-lg dark-mode:text-white focus:outline-none focus:shadow-outline">
					<img src="/img/logo.png" class="-w-40 h-16">
				</router-link>
		    ...
		    <nav :class="{'flex': open1, 'hidden': !open1}" class="flex-col flex-grow pb-4 md:pb-0 md:flex md:justify-end md:flex-row" @click="open1 = !open1">
			    ...
			    <router-link :to="'/earnmony'" class="px-2 py-2 mt-2 mr-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline" href="#">Earn Mony</router-link>
			...
			left: 9px;" class="p-1 rounded text-blue-600 text-xs ml-2 absolute">{{ cartCount }}</span> <svg style="height: 28px;width: 28px;" aria-hidden="true" focusable="false" data-prefix="far" data-icon="shopping-cart" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><path fill="currentColor" d="M551.991 64H144.28l-8.726-44.608C133.35 8.128 123.478 0 112 0H12C5.373 0 0 5.373 0 12v24c0 6.627 5.373 12 12 12h80.24l69.594 355.701C150.796 415.201 144 430.802 144 448c0 35.346 28.654 64 64 64s64-28.654 64-64a63.681 63.681 0 0 0-8.583-32h145.167a63.681 63.681 0 0 0-8.583 32c0 35.346 28.654 64 64 64 35.346 0 64-28.654 64-64 0-18.136-7.556-34.496-19.676-46.142l1.035-4.757c3.254-14.96-8.142-29.101-23.452-29.101H203.76l-9.39-48h312.405c11.29 0 21.054-7.869 23.452-18.902l45.216-208C578.695 78.139 567.299 64 551.991 64zM208 472c-13.234 0-24-10.766-24-24s10.766-24 24-24 24 10.766 24 24-10.766 24-24 24zm256 0c-13.234 0-24-10.766-24-24s10.766-24 24-24 24 10.766 24 24-10.766 24-24 24zm23.438-200H184.98l-31.31-160h368.548l-34.78 160z" class=""></path></svg>
			    ...
			    <div v-else class="flex justify-end items-center">
					<div class="w-20">
						<router-link :to="{ name: 'login' }" class="bg-purple-700 hover:bg-pink-700 text-white font-bold py-2 px-4 rounded" active-class="active">
							{{ $t('login') }}
						</router-link>
					</div>
					<div class="w-20">
						<router-link :to="{ name: 'register' }" class="bg-indigo-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded ml-2" active-class="active">
							{{ $t('register') }}
						</router-link>
					</div>
				</div>
